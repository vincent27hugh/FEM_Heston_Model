...
Beta=@(tau) 0;
dBeta=@(tau) 0;
Alpha=@(tau) exp(0.5*(q-1)*xmax+0.25*((q-1)^2)*tau);
dAlpha=@(tau) 0.25*((q-1)^2)*Alpha(tau);
...
% Set b as a matrix.
Fun_b=@(xx,tau) dx*(((xx-xmin)/(xmax-xmin))*(-dAlpha(tau))+dAlpha(tau));
...
for i=2:length(tau_v)
    ...
    g(:,i)=exp(0.25*(q+1)^2*tau_v(i)).*max(exp(0.5*(q-1).*x_eff)-exp(0.5*(q+1).*x_eff),0);
    w1(:,i)=w(:,i)+Phi_b(x_eff,tau_v(i));
    for j=1:length(x_eff)
        if w1(j,i)-g(j,i)<0 
            w1(j,i)=g(j,i);
        end
    end
    w(:,i)=w1(:,i)-Phi_b(x_eff,tau_v(i));
end
